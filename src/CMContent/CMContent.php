...
class CMContent extends CObject implements IHasSQL, ArrayAccess, IModule {
  ...
  public static function SQL($key=null) {
    ...
    $queries = array(
      ...
      'create table content'    => "CREATE TABLE IF NOT EXISTS Content(id INT(11) PRIMARY KEY AUTO_INCREMENT, filter VARCHAR(20), accesslevel INT(11), linktext VARCHAR(100), type VARCHAR(20), title VARCHAR(100), data TEXT, idUser INT(11), created DATETIME, updated DATETIME, deleted DATETIME, FOREIGN KEY(idUser) REFERENCES User(id));",
      'insert content'          => 'INSERT INTO Content (filter,accesslevel,linktext,type,title,data,idUser,created) VALUES (?,?,?,?,?,?,?,?);',
      ...
      'select * by type'        => 'SELECT c.*, u.username as owner FROM Content AS c INNER JOIN User as u ON c.idUser=u.id WHERE type=? AND accesslevel = 0 OR type=? AND accesslevel >= ? ORDER BY '.$order_by.' '.$order_order.';',
      'select *'                => 'SELECT c.*, u.username as owner FROM Content AS c INNER JOIN User as u ON c.idUser=u.id WHERE accesslevel = 0 OR accesslevel >= ?;',
      'update content'          => "UPDATE Content SET filter=?, accesslevel=?, linktext=?, type=?, title=?, data=?, updated=? WHERE id=?;",
     );
  ...
  public function Save() {
    $msg = null;
    if($this['id']) {
      $this->db->ExecuteQuery(self::SQL('update content'), array($this['filter'], $this['accesslevel'], $this['linktext'], $this['type'], $this['title'], $this['data'], date('Y-m-d H:i:s'), $this['id']));
      $msg = 'updated';
    } else {
   ...
   */
  public function ListAll($args=null, $accesslevel=0) {
    try {
      if(isset($args) && isset($args['type'])) {
        return $this->db->ExecuteSelectQueryAndFetchAll(self::SQL('select * by type', $args), array($args['type'],$args['type'], $accesslevel));
      } else {
        return $this->db->ExecuteSelectQueryAndFetchAll(self::SQL('select *', $args), array($accesslevel));
      }
    } catch(Exception $e) {
      echo $e;
      return null;
